...
import { calls, fakeAgentmail, signed } from "./agentmailFake";
...
describe("an event for another inbox", () => {
  it("is acknowledged but never mirrored", async () => {
    const stray = JSON.stringify({
      type: "event",
      event_type: "message.received",
      event_id: "evt_stray",
      message: {
        inbox_id: "someone-else@agentmail.to",
        thread_id: "thd_stray",
        message_id: "<stray@example.com>",
        timestamp: "2026-09-06T06:00:00Z",
        from: "Mallory <mallory@example.com>",
        to: ["someone-else@agentmail.to"],
        subject: "Not yours",
        text: "This belongs to a different inbox.",
      },
    });
    const before = (await EmailService.workspace(ORG_A, USER_OWNER_A)).threads
      .length;
    const result = await EmailWebhookService.processWebhook(
      accountId,
      await signed(stray, "msg_stray"),
      stray,
    );
    expect(result.status).toBe(200);
    const after = (await EmailService.workspace(ORG_A, USER_OWNER_A)).threads
      .length;
