...
Open `http://localhost:<PORT>` (default `3001`). The app is built into the
image, so container startup performs only configuration checks and database
migrations; follow progress with `docker compose logs -f`.
